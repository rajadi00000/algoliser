...
export default function Sidebar({
  ...
  return (
    <>
      ...
      <aside
        className={`
          hidden md:flex flex-col h-screen sticky top-0 bg-bg-surface border-r border-bg-overlay transition-all duration-300
          ${collapsed ? 'w-16' : 'w-64'}
        `}
      >
      {/* Logo / toggle */}
      ...
      <aside
        className={`
          fixed inset-y-0 left-0 z-50 w-[84vw] max-w-[320px] bg-bg-surface border-r border-bg-overlay transform transition-transform duration-300 md:hidden
      ...
      >
                ...
                {isActive && page !== 'home' && (
                  <div className="ml-4 mt-1 space-y-0.5 border-l border-bg-overlay pl-3">
                    {algos.map(algo => (
                      <button
                        key={algo.id}
                        onClick={() => handleNavigate(page, algo.id)}
                        title={algo.name}
                        className={`w-full text-left text-xs px-2 py-1.5 rounded transition-colors ${
                          currentAlgorithm === algo.id
                            ? 'text-white bg-bg-elevated'
        ...
        <div className="px-4 py-3 border-t border-bg-overlay">
          <div className="flex items-center gap-2 text-slate-600 text-xs">
            <GitBranch size={12} />
            <span>{ALGORITHM_META.length} algorithms</span>
          </div>
        </div>
      </aside>
    </>
